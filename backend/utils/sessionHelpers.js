...
const { getObjectContent } = require('../models/s3');
...
const addProjectCredentials = (
  ...
  postgres.db
    ...
    .then(() => {
      res.sendStatus(200);
      console.log(`Successfully added new project to database. The project ID is ${projectId}`);
    })
    .catch((error) => {
      res.sendStatus(500);
...
const extractLogEvents = (eventsArr) => eventsArr.filter(
  (obj) => obj.data.plugin === 'rrweb/console@1'
      && obj.data.payload.level !== 'error',
);

const extractNetworkEvents = (eventsArr) => eventsArr.filter((obj) => obj.type === 50);

const extractErrorEvents = (eventsArr) => eventsArr.filter((obj) => (
  obj.data.plugin === 'rrweb/console@1'
      && obj.data.payload.level === 'error'
));

const retrieveEventData = async (sessionId) => getObjectContent(sessionId)
...
const findSessionIds = async (projectId) => postgres.db
  .query(
